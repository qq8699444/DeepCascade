...
void TestObjectsDetectionApplication::compute_solution()
{
    ...
    while(video_input_is_available)
    {
        ...
        //SYLAR directory_input_p == true
        if(directory_input_p)
        {
            cv::Mat input_mat = cv::imread(directory_input_p->get_image_path());
        ...
        cv::Mat test_mat(input_ipl.get());
        cv::imshow("rrr", input_mat);
        cv::imshow("qwe", test_mat);
        cv::waitKey();*/
            objects_detection::set_monocular_image(input_view, input_mat);
        }
        else
        {
