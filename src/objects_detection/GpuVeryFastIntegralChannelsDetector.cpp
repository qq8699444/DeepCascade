...
{
    //Sylar
    string model_file  = ("Model/16_s/deploy.prototxt");
    string trained_file = ("Model/16_s/model.caffemodel");
    string mean_file   = ("Model/16_s/mean.binaryproto");
    string label_file   = ("Model/16_s/labels.txt");
    printf("initial small 16x16 model...\n");
    small_classifiers.push_back(Classifier(model_file, trained_file, mean_file, label_file));
    //classifier_16_s.initial(model_file, trained_file, mean_file, label_file);

    model_file  = ("Model/64_s/deploy.prototxt");
    trained_file = ("Model/64_s/model.caffemodel");
    mean_file    = ("Model/64_s/mean.binaryproto");
    label_file  = ("Model/64_s/labels.txt");
    printf("initial small 64x64 model...\n");
    small_classifiers.push_back(Classifier(model_file, trained_file, mean_file, label_file));
    //classifier_64_s.initial(model_file, trained_file, mean_file, label_file);

    model_file  = ("Model/128_s/deploy.prototxt");
    trained_file = ("Model/128_s/model.caffemodel");
    mean_file    = ("Model/128_s/mean.binaryproto");
    label_file  = ("Model/128_s/labels.txt");
    printf("initial small 128x128 model...\n");
    small_classifiers.push_back(Classifier(model_file, trained_file, mean_file, label_file));
    //classifier_128_s.initial(model_file, trained_file, mean_file, label_file);

    string model_file_64  = ("Model/64/deploy.prototxt");
    string trained_file_64 = ("Model/64/model.caffemodel");
    string mean_file_64    = ("Model/64/mean.binaryproto");
    string label_file_64   = ("Model/64/labels.txt");
    printf("initial big 64x64 model...\n");
    classifier_64.initial(model_file_64, trained_file_64, mean_file_64, label_file_64);
    //-------------------------------------------------------
                                                    ...
                                                    const std::string &image_file_path)
{
    ...
    //printf("GpuVeryFastIntegralChannelsDetector::set_image SYLARRR\n");
    if(false)
    {
...
void GpuVeryFastIntegralChannelsDetector::compute_v2()
{
    ...
    if(first_call)
    {
        printf("GpuVeryFastIntegralChannelsDetector::compute_v2 SYLAR" \
...

    {
        ...
        //SYLAR printf("GpuIntegralChannelsDetector::add_detection SYLARRRRRRR\n"); 
    ...
    //SYLAR BaseObjectsDetectorWithNonMaixmalSuppression::compute_non_maximal_suppresion()
    compute_non_maximal_suppresion();
    clock_gettime(CLOCK_REALTIME, &aEnd);
    total = ( aEnd.tv_sec - aStart.tv_sec ) + ( aEnd.tv_nsec - aStart.tv_nsec ) / BILLION ;    
    //cv::imshow("rqwe", input_rgb8_gpu_mem_mat);
    //cv::waitKey();
    //----------------- CNN clock time start--------------------
    struct timespec Start, End;
    double accum;
    clock_gettime(CLOCK_REALTIME, &Start);
    std::vector<cv::Rect> cnn_rect, cnn2_rect;
    std::vector<cv::Mat> rois, sec_rois;
    std::vector<detection_t> cnn_detection;
    int innercount =0;
    string np;
    detections_t detections = get_detections();    
    // ---------------------------------------
    // one image input at a time
    // ---------------------------------------
    detections_t::iterator detections_it = detections.begin();
    for(; detections_it != detections.end();detections_it++)
    {
        rois.clear();
        detection_t::rectangle_t box = detections_it->bounding_box;
        const int
            x1 = box.min_corner().x()+1, y1 = box.min_corner().y()+1,
            x2 = box.max_corner().x()-1, y2 = box.max_corner().y()-1;
        //int l_x1 = x1 - (((y2 - y1) - (x2 - x1)) / 2);
        int l_x1 = x1 - ((x2 - x1) / 8);
        int l_y1 = y1 - ((y2 - y1) / 8);
        if (l_x1 < 0)
            l_x1 = 0;
        if (l_y1 < 0)
            l_y1 = 0;
        //int l_x2 = x2 + (((y2 - y1) - (x2 - x1)) / 2);
        int l_x2 = x2 + ((x2 - x1) / 8) ;
        int l_y2 = y2 + ((y2 - y1) / 8);
        if(l_x2 > input_mat2.cols)
            l_x2 = input_mat2.cols ;
        if(l_y2 > input_mat2.rows)
            l_y2 = input_mat2.rows ;
        cv::Point tl = cv::Point(l_x1, l_y1);
        cv::Point br = cv::Point(l_x2, l_y2);
        cv::Point showtl = cv::Point(x1, y1);
        cv::Point showbr = cv::Point(x2, y2);
        if ( tl.x == br.x or tl.y == br.y )
            continue;
#ifdef SHOWIMAGE
        std::cout<<tl<<"  "<<br<<std::endl;
#endif
        cv::Mat roi = input_mat2(cv::Rect(tl, br));
        rois.push_back(roi);
        std::vector<Prediction> predictions;
        //std::cout<<rois.size()<<" size"<<std::endl;
        predictions = small_classifiers[0].Classify(roi);
        //if(predictions[0].second < 0.9 or predictions[0].first == "pos"){
            predictions.clear();
            //predictions = classifier_64.Classify(roi);
            //predictions[0].first == "pos" ? cnn_rect.push_back(cv::Rect(tl, br)) : cnn2_rect.push_back(cv::Rect(tl, br));
            //predictions[0].first == "pos" ? std::cout<<"pos"<<std::endl : std::cout<<"neg"<<std::endl;
            //predictions[0].first == "pos" ? np = "pos" : np = "neg";            
            predictions = classifier_64.Classify(roi);
            predictions[0].first == "pos" ? cnn_rect.push_back(cv::Rect(showtl, showbr)) : cnn2_rect.push_back(cv::Rect(showtl, showbr));            
            predictions[0].first == "pos" ? std::cout<<"pos"<<std::endl : std::cout<<"neg"<<std::endl;
            predictions[0].first == "pos" ? np = "pos" : np = "neg";
        //}
        //else{
          //  cnn2_rect.push_back(cv::Rect(showtl, showbr));
           // std::cout<<"neg"<<std::endl;
           // np = "neg";
        //}
        std::stringstream cs , ics;
        cs<<count;ics<<innercount;
        //cv::resize(roi, roi, cv::Size(64, 64));
        cv::imshow("roi", roi);
        char c = cv::waitKey();
        if(c=='n')
            cv::imwrite("false_neg/" + cs.str() + "_" + ics.str() + np + ".jpg", roi);
        else if(c=='p')
            cv::imwrite("false_pos/" + cs.str() + "_" + ics.str() + np + ".jpg", roi);
        innercount++;
    }
    BOOST_FOREACH(cv::Rect rect, cnn_rect){
        cv::rectangle(input_mat2, rect, CV_RGB(255, 0, 0), 2);
    }
    BOOST_FOREACH(cv::Rect rect, cnn2_rect){
        cv::rectangle(input_mat2, rect, CV_RGB(0, 0, 0), 2);
    }  
 
    // --------------------------------------------------------
    // multiple images input at a time
    // issue:
    // [20160504]same picture outputs different score everytime
    // --------------------------------------------------------
    /*detections_t::iterator detections_it = detections.begin();
    for(; detections_it != detections.end();detections_it++)
    {
        ...
        const int
            x1 = box.min_corner().x()+1, y1 = box.min_corner().y()+1,
            x2 = box.max_corner().x()-1, y2 = box.max_corner().y()-1;
        cv::Point tl = cv::Point(x1, y1);
        cv::Point br = cv::Point(x2, y2);
        if ( tl.x == br.x or tl.y == br.y )
            continue;
#ifdef SHOWIMAGE
        std::cout<<tl<<"  "<<br<<std::endl;
#endif
        cnn_rect.push_back(cv::Rect(tl, br));
        rois.push_back(input_mat2(cv::Rect(tl, br)));
        //std::stringstream cs , ics;
        //cs<<count;ics<<innercount;
        //cv::imwrite(cs.str() + "_" + ics.str() + neg + ".jpg", roi);
        innercount++;
    }
        // around 0.6 ms per classify
    std::stringstream cs ;
    cs<<count;

    cv::Mat result ;
    input_mat2.copyTo(result);
    std::vector<std::vector<Prediction> > predictions;
    if(!rois.empty()) 
        predictions = small_classifiers[0].Classify(rois, 1);
            //printf("use 64x64\n");
    for(size_t i = 0;i < predictions.size();i++){

#ifdef SHOWIMAGE      
        std::cout<<"first stage:"<<predictions[i][0].first<<" , "<<predictions[i][0].second <<std::endl;
#endif
        if(predictions[i][0].first == "pos" or predictions[i][0].second < 0.9){
            sec_rois.push_back(rois[i]);
            cnn2_rect.push_back(cnn_rect[i]);
            //cv::rectangle(input_rgb8_gpu_mem_mat, cnn_rect[i], CV_RGB(255, 0, 0), 2);
        }
        else{
            cv::rectangle(result, cnn_rect[i], CV_RGB(0, 0, 0), 2);
#ifdef SHOWIMAGE
            std::stringstream ics;
            ics<<innercount;
            //cv::imwrite(cs.str() + "_" + ics.str() + "_NEG_first.jpg", rois[i]);
#endif
            innercount++;
        } //or p.second < 0.9){
    }
    ...
    predictions.clear();
    if(!sec_rois.empty()) 
        predictions = classifier_64.Classify(sec_rois, 1);
    //std::cout<<predictions.size()<<" p2 size "<<std::endl;
    for(size_t i = 0;i < predictions.size();i++){
#ifdef SHOWIMAGE    
        std::cout<<"second stage:"<<predictions[i][0].first<<" , "<<predictions[i][0].second <<std::endl;
#endif
        if(predictions[i][0].first == "pos"){
            cv::rectangle(result, cnn2_rect[i], CV_RGB(255, 0, 0), 2);
#ifdef SHOWIMAGE
            std::stringstream ics;
            ics<<innercount;
            //cv::imwrite(cs.str() + "_" + ics.str() + "_POS_second.jpg", rois[i]);
#endif
            innercount++;
        }
        else{
            cv::rectangle(result, cnn2_rect[i], CV_RGB(0, 0, 0), 2);
#ifdef SHOWIMAGE
            std::stringstream ics;
            ics<<innercount;
            //cv::imwrite(cs.str() + "_" + ics.str() + "_NEG_second.jpg", rois[i]);
#endif
            innercount++;
        } 
    }*/   
    ...
    printf("%f ms process time for CNN, %f ms process time for FastCascade\n\n", accum*1000 , total*1000 );  
#ifdef SHOWIMAGE
    cv::imshow("result", input_mat2);
    //cv::imshow("result", result);
    //std::stringstream cs ;
    //cs<<count;
    //cv::imwrite(cs.str() + "_" + ".jpg", input_mat2);
    cv::waitKey();
#endif
    first_call = false;
    count++; 
